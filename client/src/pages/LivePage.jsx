...
import { Radio, Wifi, WifiOff, TrendingUp, Zap, Server } from 'lucide-react';
import {
  ComposedChart,
  Line,
  Area,
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-grid-400 text-sm mt-1">
            {selectedDevice} — real-time stream · data appears instantly when received · target {vNom} V, reference {iRated} A per phase
          </p>
        </div>
        <div
      ...
      {!current ? (
        <div className="glass-panel p-12 text-center">
          ...
          <p className="text-grid-400 text-lg font-semibold">Waiting for live data from {selectedDevice}...</p>
          <p className="text-grid-500 text-sm mt-1">
            Data appears immediately when the meter sends a reading. No polling delay.
          </p>
        </div>
      ) : (
        <>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="rounded-2xl border border-cyan-500/20 bg-gradient-to-br from-cyan-500/10 to-transparent px-5 py-4 shadow-sm">
              ...
              <p className="text-xs text-grid-500 mt-2 leading-snug">
                Compared with {vNom} V on the <strong>highest</strong> phase.
              </p>
            </div>
            <div className="rounded-2xl border border-amber-500/25 bg-gradient-to-br from-amber-500/10 to-transparent px-5 py-4 shadow-sm">
              ...
              <p className="text-xs text-grid-500 mt-2 leading-snug">
                0% in the 220–240 V comfort band. Rises toward limits (210 / 245 V).
              </p>
            </div>
            <div className="rounded-2xl border border-violet-500/20 bg-gradient-to-br from-violet-500/10 to-transparent px-5 py-4 shadow-sm">
              ...
              <p className="text-xs text-grid-500 mt-2 leading-snug">
                Peak phase current compared to {iRated} A reference.
              </p>
            </div>
            <div className="rounded-2xl border border-slate-200 bg-slate-50/80 px-5 py-4 shadow-sm flex flex-col justify-center">
              <p className="text-[10px] font-bold uppercase tracking-widest text-grid-500">Last reading</p>
              <p className="text-sm text-slate-700 mt-2 leading-relaxed data-readout">
                {new Date(current.bucket).toLocaleString()}
              </p>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
              ...
              <div className="w-full min-w-0" style={{ height: 320 }}>
                <ResponsiveContainer width="100%" height={320} minWidth={200}>
                  <ComposedChart data={history} margin={{ top: 12, right: 20, left: 4, bottom: 8 }}>
                    ...
                    <YAxis yAxisId="v" domain={[200, 255]} tick={{ fontSize: 10, fill: '#0891b2' }} tickLine={false}
                      axisLine={{ stroke: '#bae6fd' }} label={{ value: 'V (max phase)', angle: -90, position: 'insideLeft', style: { fill: '#0891b2', fontSize: 10 } }} />
                    <YAxis yAxisId="a" orientation="right" tick={{ fontSize: 10, fill: '#7c3aed' }} tickLine={false}
                      axisLine={{ stroke: '#ddd6fe' }} label={{ value: 'A (max phase)', angle: 90, position: 'insideRight', style: { fill: '#7c3aed', fontSize: 10 } }} />
                    <Tooltip contentStyle={chartTooltip} formatter={(value, name) => [typeof value === 'number' ? value.toFixed(3) : value, name]} />
                    <Legend wrapperStyle={{ fontSize: 12, paddingTop: 8 }} />
                    <ReferenceArea yAxisId="v" y1={220} y2={240} fill="#22c55e" fillOpacity={0.07} />
                    <ReferenceLine yAxisId="v" y={vNom} stroke="#475569" strokeDasharray="5 5" strokeWidth={1.2}
                      label={{ value: `${vNom} V`, fill: '#64748b', fontSize: 10, position: 'insideTopLeft' }} />
                    <Area yAxisId="v" type="monotone" dataKey="maxV" name="Max voltage (V)" stroke="#0891b2" strokeWidth={2.5}
                      fill="url(#gradV)" dot={false} isAnimationActive={false} activeDot={{ r: 5, strokeWidth: 0, fill: '#0891b2' }} />
                    <Line yAxisId="a" type="monotone" dataKey="maxA" name="Max current (A)" stroke="#7c3aed" strokeWidth={2.5}
                      dot={false} isAnimationActive={false} activeDot={{ r: 5, strokeWidth: 0, fill: '#7c3aed' }} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="relative overflow-hidden rounded-2xl border border-slate-200/80 bg-white/70 shadow-[0_20px_60px_-24px_rgba(15,23,42,0.25)] p-6 min-w-0 backdrop-blur-sm">
              ...
              <p className="text-xs text-slate-500 mb-5">Total real power with timestamps — updated in real-time.</p>
              <div className="w-full min-w-0" style={{ height: 320 }}>
                <ResponsiveContainer width="100%" height={320} minWidth={200}>
                  <ComposedChart data={history} margin={{ top: 12, right: 12, left: 4, bottom: 8 }}>
                    ...
                    <Area type="monotone" dataKey="ap" name="Active power (W)" stroke="#0284c7" strokeWidth={2.5}
                      fill="url(#gradP)" dot={false} isAnimationActive={false} />
                  </ComposedChart>
                </ResponsiveContainer>
              </div>
            </div>
